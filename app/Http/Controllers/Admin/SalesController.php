...
class SalesController extends Controller
{
    ...
    function getList(Request $request) {
        $page = $request->page - 1;
        $data = SalesOrder::orderBy("id" , "DESC")->with('province')->with('district')->with('ward');
        if(@$request->name != '' ){
        ...
        foreach ($data as &$item) {
            ...
            $product = Products::with('parent')->with('origins')->with('trademarks')->where('products.id',@$item->product_id);
            ...
            if ($item->discount != '') {
                $item->total = ($product['pricenew'] * $item->quantity) * (100 - $item->discount) / 100;
            } else {
                $item->total = ($product['pricenew'] * $item->quantity);
            }
