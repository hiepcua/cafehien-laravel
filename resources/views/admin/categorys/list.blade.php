                        ...
                        <table class="table table-hover table-custom spacing5 mb-0">
                            ...
                            <tbody>
                                <tr ng-repeat="content in data">
                                    ...
                                    <td>
                                        <span ng-if="content.edit == 1"><img src="((content.icon))" ng-if="content.icon" data-toggle="tooltip" data-placement="top" title="Avatar Name" alt="Avatar" class="rounded-circle h-100"> </span>
                                        <div ng-if="content.edit == 2" class="input-group">
                                            <ul id="images">
                                                <li>   
                                                    <input class="input_image" type="hidden" id="chooseImage_input((content.id))" ng-model="content.icon" value="((content.icon))">
                                                    <div id="chooseImage_div((content.id))" style="((content.icon != '' ? '' : 'display: none;')) ">
                                                        <img src="((content.icon))" id="chooseImage_img((content.id))" style="max-width: 150px; max-height:150px; border:dashed thin;"></img>
                                                    </div>
                                                    <div id="chooseImage_noImage_div((content.id))" style="((content.icon == '' ? '' : 'display: none;'))width: 150px; border: thin dashed; text-align: center; padding:70px 0px;">
                                                        No image
                                                    </div>
                                                    <br/>
                                    ...
                                    <td>
                                        <span ng-if="content.edit == 1">((content.parent.category ? content.parent.category : ""))</span>
                                        <select ng-if="content.edit == 2" class="custom-select edit-in-table" ng-model="content.category_id" >
                                            <option value="">Chọn Danh Mục</option>
                                            <option ng-repeat="res in dataParent" value="((res.id))">((res.category))</option>
                                        </select>
                                    </td>
                                    <td>
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group c_form_group">
                                            ...
                                            <div class="input-group">
                                                <select class="custom-select" ng-model="category_parent">
...
customInterpolationApp.controller('AppController', function($scope, $http , $q) {
         ...
         $scope.run_action = function(){
            ...
            $scope.pageLoad = 1;
            $http.get("{{route('admin.getListCategory')}}?page="+$scope.pageLoad+"&category="+$scope.nameCondition).then(function(data, status) {
        ...
        $scope.updateConfirm = function(i){
            Swal.fire({
                ...
                console.log($("#chooseImage_input"+i.id).val());
                var body = { 
                    _token: CSRF_TOKEN ,
                    id : i.id,
                    category_id : i.category_id,
                    icon : $("#chooseImage_input"+i.id).val(),
                    category : i.category
                };

                $http.post('/admin/categorys/edit/'+i.id, body).then(function(data, status) {
                    i.edit = 1;
        ...
        $scope.addConfirm = function(){
            ...
            Swal.fire({
                ...
                var body = { 
                    ...
                    icon : $("#chooseImage_inputnew").val()
                };
                $http.post("{{route('admin.addCategory')}}", body).then(function(data, status) {
                    $scope.run_action();
                    $('.new-project-modal').modal("hide");
                    $scope.category_parent = '';
                    $scope.name_category = '';
