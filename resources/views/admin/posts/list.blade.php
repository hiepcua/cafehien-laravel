...
@section('parentPageTitle', 'Bài Viết')
        ...
        <div class="col-xl-7 col-md-7 col-sm-12 text-md-right">
            <div class="d-flex align-items-center justify-content-md-end mt-4 mt-md-0 flex-wrap vivify pullUp delay-550">
                <div class="border-right pr-4 mr-4 mb-2 mb-xl-0">
                    ...
                    <h5 class="mb-0">((totalUsers)) Bài Viết</h5>
                </div>
                ...
                <div class="tab-content mt-0">
                    ...
                    <div class="form-group c_form_group bg-white">
                        <div class="input-group">
                            <input type="text" ng-model="nameCondition" ng-keydown="changeKey($event)" class="form-control" placeholder="Tên Bài Viết">
                        ...
                        <table class="table table-hover table-custom spacing5 mb-0">
                            ...
                            <tbody>
                                <tr ng-repeat="content in data">
                                    ...
                                    <td>
                                        <a href="/admin/posts/edit/((content.id))" title="">((content.title))</a>
                                        <p class="mb-0">((content.slug))</p>
                                    </td>
                                    <td>
                                        <span>
                                            <label class="switch">
                                    ...
                                    <td>
                                        <span ng-if="content.category_id != 2">
                                            <label class="switch">
                                                <input type="checkbox" ng-change="changePopup(content)" ng-model="content.is_popup" ng-true-value="1" ng-false-value="0">
                                                <span class="slider round"></span>
                                            </label>
                                    ...
                                    <td>
                                        <a href="/admin/posts/edit/((content.id))" class="btn btn-primary theme-bg btn-sm"><i class="fa fa-edit"></i></a>
                                        <button class="btn btn-danger btn-sm" ng-if="content.parent.id != 2"  ng-click="delete(content.id)" ><i class="fa fa-trash"></i></button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
customInterpolationApp.controller('AppController', function($scope, $http , $q) {
        ...
        $scope.run_action = function(){
            ...
            $scope.pageLoad = 1;
            $http.get("{{route('admin.getListPosts')}}?page="+$scope.pageLoad+"&title="+$scope.nameCondition).then(function(data, status) {
        ...
        $scope.changePopup = function(updateItem){
            if (updateItem.is_popup === 1) {
                $.ajax({
                    url: "/admin/posts/active-popup/"+updateItem.id,
                    type: 'GET',
                    dataType: 'json',
                ...
                $.ajax({
                    url: "/admin/posts/deactive-popup/"+updateItem.id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(res) {
                        $scope.changePage($scope.pageLoad);
        ...
        $scope.changeStatus = function(updateItem){
            if (updateItem.is_active === 1) {
                $.ajax({
                    url: "/admin/posts/active-status/"+updateItem.id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(res) {
                    }
                ...
                $.ajax({
                    url: "/admin/posts/deactive-status/"+updateItem.id,
                    type: 'GET',
                    dataType: 'json',
                    success: function(res) {
                    }
        ...
        $scope.newDataConfirm = function(e){
            ...
            Swal.fire({
                ...
                var body = { 
                    _token: CSRF_TOKEN ,
                    title : $scope.name_product,
                    slug : $scope.slug_product,
                    image : $("#chooseImage_inputnew").val()
                };
                $http.post("{{route('admin.addPosts')}}", body).then(function(data, status) {
                    console.log(data);
                    if (data.data.errors === 0 ) {
                        window.location.replace("/admin/posts/edit/"+data.data.data.id);
                    } else {
                        Swal.fire({
        ...
        $scope.delete = function(id_delete){
            Swal.fire({
                ...
                $.ajax({
                    url: "/admin/posts/delete/"+id_delete,
                    type: 'GET',
                    dataType: 'json',
                    success: function(res) {
                        Swal.fire({
